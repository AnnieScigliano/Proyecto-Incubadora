...
function config:get_wifi_with_error()
  local body, code, _, _, message = http_request_methods.http.request(http_request_methods.apiendpoint .. "wifi")
  http_request_methods.inspect(print("\nBody de la petición GET: \n " .. body))
  http_request_methods.assert.are.equal(code, 200, "La primera petición debería devolver un error")
end

function config:get_wifi()
  local body, code, _, status, _ = http_request_methods.http.request(http_request_methods.apiendpoint .. "wifi")
...
function config:get_wifi_with_5s()
  os.execute("sleep 5")
  local body, code, _, _, message = http_request_methods.http.request(http_request_methods.apiendpoint .. "wifi")
  http_request_methods.assert.are.equal(code, 200, "Error en la tercera petición GET")
  http_request_methods.inspect(print("\nBody de la tercera petición GET : \n " .. body))
end

function config:get_wifi_without_tmr()
  local body, code, _, _, message = http_request_methods.http.request(http_request_methods.apiendpoint .. "wifi")
  http_request_methods.assert.are.equal(code, 200, "Error en la cuarta petición GET")
...
function config:set_config_ok()
  ...
  local previous_config = config.http_request_methods:get_and_assert_200("config")

  local success, _ = pcall(function()
    config.http_request_methods:post_and_assert_201("config", self.tables.configs_to_test_numbers.config_40_50)

    -- waiting for the apply
    os.execute("sleep 1")

    -- get before change confing
    local new_config = config.http_request_methods:get_and_assert_200("config")
  ...
  config.http_request_methods
      :post_and_assert_201("config", http_request_methods.JSON:encode(default_config))
  config:assert_defconfig()
end

function config:set_max_fail()
  default_config = config:assert_defconfig()
  local body = config.http_request_methods
      :post_and_assert_400("config", self.tables.configs_to_test_numbers.config_50_40)
  default_config = config:assert_defconfig()
...
function config:set_min_rotation_time_fail()
  ...
  local body = config.http_request_methods
      :post_and_assert_400("config", self.tables.configs_to_test_numbers.config_1_40_50)
  default_config = config:assert_defconfig()
...
function config:set_negative_temps_value_fails()
  default_config = config:assert_defconfig()
  local body_negative_1 = config.http_request_methods
      :post_and_assert_400("config", self.tables.configs_to_test_numbers.config_m10_50)
  default_config = config:assert_defconfig()
  local body_negative_2 = config.http_request_methods
      :post_and_assert_400("config", self.tables.configs_to_test_numbers.config_10_m10)
  default_config = config:assert_defconfig()
...
function config:set_noise_in_parameters()
  default_config = config:assert_defconfig()
  body_noise_1 = config.http_request_methods
      :post_and_assert_400("config", self.tables.config_to_test_str.noise_in_min_temp)
  default_config = config:assert_defconfig()
  body_noise_2 = config.http_request_methods
      :post_and_assert_400("config", self.tables.config_to_test_str.noise_in_rotation_duration)
  default_config = config:assert_defconfig()
  body_noise_3 = config.http_request_methods
      :post_and_assert_400("config", self.tables.config_to_test_str.noise_in_max_temperature)
  default_config = config:assert_defconfig()
...
function config:assert_defconfig()
  ...
  http_request_methods.assert.are.equal(default_config.passwd, "12345678")
