...
describe("[#] API REST TDD", function()
	...
	describe(test.colors("%{green}[#] Get the current configuration"), function()
	...
	describe(test.colors("%{green}[#] Sets the configuration to default"), function()
		test.config:set_config_ok()
		it(test.colors("%{red}[!] should set the configuration to default"), function()
		end)
	end)
	...
	describe(test.colors("%{green}[#] Configuration tests"), function()
		...
		it(test.colors("%{red}[!] Setting min_temp setting higher than max_temp should fail"), function()
		...
		it(test.colors("%{red}[!] Setting a minimum value in rotation_duration should fail"), function()
			assert.are.equal(body_req_rotation_time, "Error in setting rotation_duration")
		end)
		--	set negative temps
		local body_req_min_temp_negative, body_req_max_temp_negative = test.config
				:set_negative_temps_value_fails()
		...
		it(test.colors("%{red}[!]Setting temperatures with negative values should fail"), function()
		...
		it(test.colors("%{red}[!] Setting strings to numeric values should fail"), function()
